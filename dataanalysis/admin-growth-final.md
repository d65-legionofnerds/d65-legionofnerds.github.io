---
title: Admin Growth (Publication Candidate)
layout: default
nav_exclude: true
search_exclude: true
noindex: true
...
**A Legion of Data Nerds analysis of how D65's administrative footprint has evolved over the past decade, what it costs today, and what right-sizing it could save — using two independent data sources to make the case rigorously.**

## Overview

Over the past decade, District 65 has dramatically expanded its administrative compensation footprint while serving a shrinking student population. This page combines two independent public data sources to make that case unambiguously:

1. **The District's own Annual Financial Report (AFR), filed annually with ISBE.** This captures every employee in admin functions regardless of salary, including admin support staff who make less than $75,000.
2. **State public-disclosure compensation reports under PA 96-0434 (TRS) and PA 97-0609 (IMRF).** These provide the only year-by-year roster counts for higher-paid administrators.

We use the **AFR for dollar trends** (it captures the full pool without any salary cutoff) and **PA disclosures with a CPI-indexed threshold for headcount trends** (the threshold-indexing makes 2016 and 2026 comparable in real-dollar terms, removing the inflation drift that mechanically inflates headcount under the unindexed $75,000 statutory threshold). All dollar figures on this page are in **2026 dollars** using BLS CPI-U.
...
- **The AFR admin compensation pool grew from $16.56M to $19.99M in real (2026) dollars — a 21% real increase.** Excluding school administration (principals), the central-office and support pool grew from $9.37M to $12.49M (+33% real).
- **General Administration alone (function 2300, which captures the superintendent, assistant superintendents, curriculum directors and coordinators) more than doubled in real terms — from $2.40M to $4.88M (+103% real).**
- **Per-student real admin spending rose from $2,201 to $3,401 — a 55% real increase per pupil.**
- **Higher-paid admin headcount (CPI-indexed PA roster) grew from 75 to 94 (+25%) — and per-1,000-student density rose from 10.0 to 16.7 (+67%).**
- **TRS Admin (the certificated central-office category) more than doubled, 11 → 24 (+118%)** — independent of any threshold methodology.

We detail the growth below, but want to emphasize the savings opportunity at the top. The chart below shows recent enacted/proposed cuts (dark grey), the District's own April 20, 2026 admin-cut proposal (orange), and three Legion right-sizing scenarios (red shades) — all expressed both in dollars and as a percentage of the FY25 AFR admin compensation pool.
...
4. **The District's current cost-cutting focus is heavily on student-facing services** (counselors, librarians, school buildings, iPad cuts) while the central-administration buildup of the past decade has received much less scrutiny. The District's own April 20, 2026 deck identified $8.3M in admin cuts available — but presented the proposal 19 days after the contractual deadline that would have made it actionable for FY27, deferring those cuts to FY28 at the earliest.

5. **Comparable K-8 districts operate with substantially leaner administrative structures.** Larry Gavin's *Evanston RoundTable* reporting notes that D65's −25% enrollment decline since FY 2019 is the steepest among 21 nearby K-8 districts; the next-largest decline was 14.3%. Meanwhile, D65 spends substantially more per pupil on central administration than peer districts.

6. **Achievement has not improved.** D65 math proficiency on the IAR went from 44.8% meeting benchmark in 2018 to 42.3% in 2024 (per Gavin's March 2026 *RoundTable* analysis). Whatever the administrative buildup has produced, it has not produced the academic outcomes its hiring case would have presumably promised.

The Legion of Data Nerds publishes this data to inform public discussion. We encourage residents, board members, and District leadership to consider it alongside other operational and educational priorities. Additional D65 source documents are available on the [D65 BoardBook archive](https://meetings.boardbook.org/Public/Organization/1247) and the [D65 Data Dashboard](https://data.district65.net/). Independent journalism is available in the [Evanston RoundTable](https://evanstonroundtable.com/).
...
### A note on the "more dramatic" $12.7M figure on the [companion page](admin-growth.html)

An earlier Legion analysis (preserved on the companion [Admin Growth (unindexed)](admin-growth.html) page) computed right-sizing scenarios on the basis of the unindexed PA-disclosed headcount of 153 administrators in FY26, yielding $11.2M and $12.7M figures. **This page uses a more conservative methodology that nonetheless preserves the core conclusion**: even under the strict apples-to-apples CPI-indexed framework, with right-sizing per-cut compensation set to the District's own implied average ($129K), several million dollars in annual admin savings remain accessible — and the District itself has identified $8.3M (41.5% of the FY25 admin pool). The methodology on this page is harder to attack on inflation-drift grounds, harder to attack on threshold-cherry-picking grounds, and harder to attack on per-cut-comp grounds; the case for material admin savings holds up under every defensible analytical choice.
...
- **Not included.** Paraprofessionals (~200 D65 staff, comp typically below $30,000), counselors, librarians, classroom teachers, and support staff below the indexed threshold are not in the PA-disclosed pool.
...
The chart below is the marquee data on this page. It shows D65's total compensation paid to administrators across all four major admin function codes, in real (2026) dollars, from FY2012 through FY2025. **No salary threshold is applied** — every employee whose salary or benefits hit these function codes is counted.

<iframe src="assets/admin_growth_final_afr_pool_stacked.html" width="100%" height="650" frameborder="0"></iframe>
...
<iframe src="assets/admin_growth_final_imrf_categories.html" width="100%" height="650" frameborder="0"></iframe>
...
The two-counselor layoff saves roughly the cost of a single mid-level administrator position. The librarian reassignments save effectively nothing in dollars. **Closing Bessie Rhodes and Kingsley together (the active 2026 plan) saves about $3.5M annually — comparable to the Legion's peer-median right-sizing scenario** ($3.4M). Even the District's own April 20 proposal is bigger than every closure measure combined.
...
- **Data sources.** Year-by-year admin headcount and individual-level compensation come from PA 96-0434 (TRS administrators) and PA 97-0609 (IMRF employees with comp packages over $75K), filed annually with ISBE and indexed in [foiagras](https://foiagras.com). Total admin compensation pool comes from the District's Annual Financial Report (AFR), which D65 also files annually with ISBE and which we obtained from [districtvitals.com](https://ccsd65.districtvitals.com/data#sec-afr-excel) and our local archive at [data/afr/](data/afr/).
...
- **Inflation adjustment** uses BLS CPI-U All Urban Consumers (US city average) annual averages, with 2026 as the base year. CPI values are stored in [data/cpi_history.csv](data/cpi_history.csv); the SY25-26 value is estimated from monthly Jan–Nov 2025 BLS data.
...
- **PA 097-0609 (IMRF) reports include only IMRF-participating employees whose compensation package (salary plus benefits) exceeds $75,000.** This page's CPI-indexed threshold is *additionally* applied on top of that legal threshold. The unindexed page ([Admin Growth (unindexed)](admin-growth.html)) uses only the legal $75,000 threshold.
...
- **Enrollment** for years not present in the website's existing enrollment dataset (SY2015-16, SY2016-17, SY2025-26) is sourced from the District's annual *Opening of Schools* reports on foiagras. The SY2025-26 enrollment is from [D65's Data Dashboard](https://data.district65.net/). **Note on enrollment-counting differences**: this page uses the K-8 + PreK-with-IEP figure (5,625 for SY25-26) consistent with how D65's *Opening of Schools* reports count students. Larry Gavin's *RoundTable* analyses use ISBE's "total served" figure (5,941 for SY25-26), which adds students in early-childhood programs, Park School, Rice Center, and JEH that are not in graded K-8 classrooms. Both figures are correct under their respective methodologies.

### Companion analyses

- **[Admin Growth (unindexed)](admin-growth.html)** — uses only the unindexed $75,000 PA threshold. Larger headline numbers (153 admin in FY26, +104% headcount; $12.7M / $11.2M right-sizing scenarios). Methodologically less conservative; included for transparency and historical comparison.
- **[Admin Growth (indexed-only)](admin-growth-indexed.html)** — uses CPI-indexed PA threshold throughout, including for right-sizing math. Smaller headline numbers but methodologically purer than the unindexed version. This page (admin-growth-final.md) goes further by using the AFR pool as the right-sizing denominator rather than the PA-indexed pool.

---

## Acknowledgements

This analysis was prepared by the Legion of Data Nerds with the assistance of [Claude.ai](https://claude.ai) (Anthropic), which helped with extracting and reconciling data across the multi-year PA compensation reports and AFR Expenditures sheets, building the visualization pipeline, and drafting this page. All source data is from public District 65 board meeting documents indexed on [foiagras](https://foiagras.com) and from publicly-filed ISBE Annual Financial Reports. All editorial decisions, methodology choices, and conclusions are the authors'.

*Underlying CSV data is available at [data/d65_admin_comp_combined.csv](data/d65_admin_comp_combined.csv) (PA disclosures) and [data/afr_admin_pool_summary.csv](data/afr_admin_pool_summary.csv) (AFR pool). Source AFR Excel files are in [data/afr/](data/afr/). Build scripts for the dataset and visualizations are in [build_admin_growth_data.py](build_admin_growth_data.py), [build_afr_admin_pool.py](build_afr_admin_pool.py), and [build_admin_growth_final_charts.py](build_admin_growth_final_charts.py).*
